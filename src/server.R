...
shinyServer(function(input, output, session) {
  consumer_key<-"0pnUJh7cX27ZsJB0kVS6qK98t"
  consumer_secret<-"Kk1HPGDKKQtPHX4bDsamv3rLxXYndmMe2KcK18ovW0376G3kLP"
  access_token<-"761308928303128577-5NKlD6rwNeb3q9rtAlfXa36xyuosCXu"
  access_secret<-"YjgY2tt4De73R2Jp3EcQE9gj1rya6F0ZNGupnAiR9EWKw"
  ...
  observe({
    ...
    tweets_result = ""
    tweets_result = searchTwitter("#brexit",n=1000)
    for (tweet in tweets_result){
      print(paste(tweet$screenName, ":", tweet$text))
      vector_users <- c(vector_users, as.character(tweet$screenName));
      if (grepl("referendum", tweet$text, ignore.case = TRUE) == TRUE |grepl("plebiscite", tweet$text, ignore.case = TRUE)|  grepl("vote", tweet$text, ignore.case = TRUE) | grepl("poll", tweet$text, ignore.case = TRUE)){
        count_positive = count_positive + 1
        print("positivo")
        vector_sentiments <- c(vector_sentiments, "Positive")
        positive_text <- c(positive_text, as.character(tweet$text))
      } else if (grepl("debt", tweet$text, ignore.case = TRUE)| grepl("immigration", tweet$text, ignore.case = TRUE)| grepl("job", tweet$text, ignore.case = TRUE)| grepl("crisis", tweet$text, ignore.case = TRUE)| grepl("depreciated", tweet$text, ignore.case = TRUE)| grepl("trade", tweet$text, ignore.case = TRUE)) { 
